...
def generate_test_description(ready_fn):
    ...
    cmd = ['/opt/workspace/build/test_rclcpp/test_services_server_cpp']
    if '':
      cmd += ''.split(' ')
    env = None
    if '':
...
class TestTwoExecutables(unittest.TestCase):
    ...
    def test_services_cpp(self, executable_2):
...
class TestTwoExecutablesAfterShutdown(unittest.TestCase):
    ...
    def test_services_cpp(self, proc_info):
        """Test that both executables finished cleanly."""
        launch_testing.asserts.assertExitCodes(
            proc_info,
            [launch_testing.asserts.EXIT_OK],
            'test_services_client_cpp'
        )
        launch_testing.asserts.assertExitCodes(
            proc_info,
            process='test_services_server_cpp'
        )
