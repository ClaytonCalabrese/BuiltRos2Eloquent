...
def generate_test_description(ready_fn):
    ...
    cmd = ['/opt/workspace/build/test_rclcpp/test_client_scope_consistency_server_cpp']
    if '':
      cmd += ''.split(' ')
    env = None
    if '':
    ...
    executable_1 = ExecuteProcess(
        cmd=cmd,
        name='test_client_scope_consistency_server_cpp',
        env=env,
        sigterm_timeout=LaunchConfiguration('sigterm_timeout', default=30)
    )
    launch_description.add_action(executable_1)

    cmd = ['/opt/workspace/build/test_rclcpp/test_client_scope_consistency_client_cpp']
    if '':
      cmd += ''.split(' ')
    env = None
    if '':
    ...
    executable_2 = ExecuteProcess(
        cmd=cmd,
        name='test_client_scope_consistency_client_cpp',
        env=env,
        sigterm_timeout=LaunchConfiguration('sigterm_timeout', default=30)
    )
...
class TestTwoExecutables(unittest.TestCase):
    ...
    def test_client_scope_consistency_cpp(self, executable_2):
...
class TestTwoExecutablesAfterShutdown(unittest.TestCase):
    ...
    def test_client_scope_consistency_cpp(self, proc_info):
        ...
        launch_testing.asserts.assertExitCodes(
            proc_info,
            process='test_client_scope_consistency_server_cpp'
        )
