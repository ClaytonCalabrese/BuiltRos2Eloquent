...
def generate_test_description():
    ...
    launch_description.add_action(
        SetEnvironmentVariable('RMW_IMPLEMENTATION', 'rmw_fastrtps_dynamic_cpp'))
    ...
    showimage_executable = '/opt/workspace/build/image_tools/showimage'
    showimage_name = 'test_showimage'

    showimage_node = Node(
        node_executable=showimage_executable,
        node_name=showimage_name,
    ...
    cam2image_executable = '/opt/workspace/build/image_tools/cam2image'
    cam2image_name = 'test_cam2image'

    cam2image_node = Node(
        node_executable=cam2image_executable,
        node_name=cam2image_name,
    ...
    def test_reliable_qos(self, proc_output, showimage_node, cam2image_node):
        ...
        proc_output.assertWaitFor(
            expected_output=launch_testing.tools.expected_output_from_file(
                path='/opt/workspace/src/ros2/demos/image_tools/test/showimage'
            ), process=showimage_node, output_filter=output_filter, timeout=10
        )
        proc_output.assertWaitFor(
            expected_output=launch_testing.tools.expected_output_from_file(
                path='/opt/workspace/src/ros2/demos/image_tools/test/cam2image'
