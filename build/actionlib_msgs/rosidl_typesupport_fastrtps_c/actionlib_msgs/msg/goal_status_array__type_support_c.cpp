...
#include "actionlib_msgs/msg/goal_status_array__rosidl_typesupport_fastrtps_c.h"
...
#include "actionlib_msgs/msg/goal_status__functions.h"  // status_list
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: status_list
  {
    ...
    if (!actionlib_msgs__msg__GoalStatus__Sequence__init(&ros_message->status_list, size)) {
      return "failed to create array for field 'status_list'";
    }
    auto array_ptr = ros_message->status_list.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _GoalStatusArray__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_actionlib_msgs__msg__GoalStatusArray(
    full_bounded, 0);
}
