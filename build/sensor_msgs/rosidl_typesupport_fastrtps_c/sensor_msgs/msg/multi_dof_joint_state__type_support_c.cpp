...
#include "sensor_msgs/msg/multi_dof_joint_state__rosidl_typesupport_fastrtps_c.h"
...
#include "geometry_msgs/msg/transform__functions.h"  // transforms
  ...
  void * untyped_ros_message)
{
  ...
  _MultiDOFJointState__ros_msg_type * ros_message = static_cast<_MultiDOFJointState__ros_msg_type *>(untyped_ros_message);
  // Field name: header
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
  ...
  // Field name: twist
  {
    ...
    if (!geometry_msgs__msg__Twist__Sequence__init(&ros_message->twist, size)) {
      return "failed to create array for field 'twist'";
    }
    auto array_ptr = ros_message->twist.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
  ...
  // Field name: wrench
  {
    ...
    if (!geometry_msgs__msg__Wrench__Sequence__init(&ros_message->wrench, size)) {
      return "failed to create array for field 'wrench'";
    }
    auto array_ptr = ros_message->wrench.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
{
  return max_serialized_size_sensor_msgs__msg__MultiDOFJointState(
    full_bounded, 0);
}
