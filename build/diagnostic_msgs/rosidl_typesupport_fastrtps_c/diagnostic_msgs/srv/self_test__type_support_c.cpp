...
#include "diagnostic_msgs/srv/self_test__rosidl_typesupport_fastrtps_c.h"
...
{
  return max_serialized_size_diagnostic_msgs__srv__SelfTest_Request(
    full_bounded, 0);
}
...
#include "diagnostic_msgs/msg/diagnostic_status__functions.h"  // status
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: status
  {
    ...
    if (!diagnostic_msgs__msg__DiagnosticStatus__Sequence__init(&ros_message->status, size)) {
      return "failed to create array for field 'status'";
    }
    auto array_ptr = ros_message->status.data;
    for (size_t i = 0; i < size; ++i) {
...
{
  return max_serialized_size_diagnostic_msgs__srv__SelfTest_Response(
    full_bounded, 0);
}
