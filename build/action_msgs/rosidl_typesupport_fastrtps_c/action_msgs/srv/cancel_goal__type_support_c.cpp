...
#include "action_msgs/srv/cancel_goal__rosidl_typesupport_fastrtps_c.h"
...
#include "action_msgs/msg/goal_info__functions.h"  // goal_info
...
{
  return max_serialized_size_action_msgs__srv__CancelGoal_Request(
    full_bounded, 0);
}
...
// already included above
// #include "action_msgs/msg/goal_info__functions.h"  // goals_canceling
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: goals_canceling
  {
    ...
    if (!action_msgs__msg__GoalInfo__Sequence__init(&ros_message->goals_canceling, size)) {
      return "failed to create array for field 'goals_canceling'";
    }
    auto array_ptr = ros_message->goals_canceling.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
{
  return max_serialized_size_action_msgs__srv__CancelGoal_Response(
    full_bounded, 0);
}
